...
class WMSClient(object):
    ...
    def _check_resp(self, resp, url):
        if not resp.headers.get('Content-type', 'image/').startswith('image/'):
            ...
            if len(data) == log_size:
                data += '... truncated'
            log.warn(
                "no image returned from source WMS: %s, response was: %s" % (url, data))
            raise SourceError(
                'no image returned from source WMS: %s' % (url, ))
    ...
    def combined_client(self, other, query):
        ...
        new_req.params.layers = new_req.params.layers + \
            other.request_template.params.layers
...
class WMSInfoClient(object):
    ...
    def _get_transformed_query(self, query):
        ...
        req_srs = query.srs
        req_bbox = query.bbox
        req_coord = make_lin_transf(
            (0, 0, query.size[0], query.size[1]), req_bbox)(query.pos)
        ...
        info_aratio = (info_bbox[3] - info_bbox[1]) / \
            (info_bbox[2] - info_bbox[0])
